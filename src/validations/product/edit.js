...
const validateCountImages = (countImagesUpload, countImagesUploaded) => {
  if (validator.isEmpty(countImagesUpload)) {
    return message.COUNT_IMAGES_DOSE_NOT_EXIST;
  }

  if (validator.isEmpty(countImagesUploaded)) {
    return message.COUNT_IMAGES_DOSE_NOT_EXIST;
  }
...
const validate = (info) => {
  ...
  const resultValidateCountImages = validateCountImages(
    info.count_images_upload,
    info.count_images_uploaded
  );
