...
export default function Edit() {
  ...
  async function getPermission() {
    ...
    let resultGetPermission = await sendRequestGetPermission(permissionID);
    ...
    setPermission(resultGetPermission);
  }

  async function sendRequestGetPermission(permissionID) {
    try {
      const result = await httpHelper.getRequest(
        `${process.env.REACT_APP_DOMAIN_API}/api/v1/dashboard/auth/permission/${permissionID}`,
        getHeaderForRequest()
      );
      return {
        result: true,
  ...
  async function sendForm() {
    ...
    spinnerHelper.showSpinner(spinnerHelper.CLASS_CONTENT_WRAPPER_TAG);
    let resultEditPermission = await sendRequestEditPermission(formData);
    spinnerHelper.removeSpinner(spinnerHelper.CLASS_CONTENT_WRAPPER_TAG);
    if (!resultEditPermission.result) {
      return;
    }

    toast.success("دسترسی ویرایش شد.", {
      ...toastHelper.getOptionErrorToast(),
  ...
  async function sendRequestEditPermission(bodyRequest) {
    try {
          ...
          <div className="card">
            <div className="card-body">
              <div className="text-left pb-2">
                <Link to="/permission/all" className="btn btn-primary">
                ...
                >
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      id="persian_name"
                      className="form-control"
                      defaultValue={permission.persian_name}
                      required
                    />
                  </div>
                  <div className="form-group">
